...
![GPU memory hierarchy on the NVIDIA Tesla V100, showing per-SM storage (register file, shared memory, L1 cache, and constant caches) together with device-wide L2 cache and global memory.](image_1.png "GPU memory hierarchy on the NVIDIA Tesla V100, showing per-SM storage (register file, shared memory, L1 cache, and constant caches) together with device-wide L2 cache and global memory. Source: Cornell Virtual Workshop, [“GPU Memory Levels.”](https://cvw.cac.cornell.edu/gpu-architecture/gpu-memory/memory_levels)")
